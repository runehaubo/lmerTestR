...
The computational challange is essentially to evaluate the denominator in the expression for $\hat\nu(\hat\tau)$, which amounts to computing the gradient $f{'}(\hat\tau)$ and variance-covariance matrix of the variance parameter vector $\mathsf{V}(\hat\tau)$. We will now address these challanges in turn starting with the latter.
...
(parlist <- list(beta=fixef(model),
                ...
                sigma=sigma(model)))
```
and the consumer random effect standard deviation can be retreived with 
```{r}
...
The convential $F$-statistic for this hypothesis reads
$$F = \frac{1}{q} (L\hat\beta)^\top (L \mathsf V_\hat\tau(\hat\beta)L^\top)^{-1} (L\hat\beta)$$
which is seen to reduce to the square of the $t$-statistic considered above for $q=1$. Dispite its name, this statistic only follows an exact $F$-distribution with $q$ numerator degrees of freedom and known denominator degrees of freedom in special cases (such as orthogonal or balanced designs.) 
...
We then compute the eigen-decompostion $\mathsf V(L\hat\beta) = P^\top D P$ and extract the rank, eigenvectors and eigenvalues:
```{r}
...
The new constrast vectors are then $\tilde L = P^\top L$
```{r}
...
[1] Kuznetsova, A., Brockhoff, P., & Christensen, R. (2017). "lmerTest Package: Tests in Linear Mixed Effects Models." Journal of Statistical Software, 82(13), 1--26. doi:http://dx.doi.org/10.18637/jss.v082.i13

[2] Schaalje GB, McBride JB, Fellingham GW (2002). "Adequacy of Approximations to Distributions of Test Statistics in Complex Mixed Linear Models." Journal of Agricultural, Biological, and Environmental Statistics, 7(4), 512–524. doi:10.1198/108571102726.

[3] Satterthwaite FE (1946). "An Approximate Distribution of Estimates of Variance Components." Biometrics Bulletin, 2(6), 110-–114. doi:10.2307/3002019.

[4] Bates DM, Mäechler M, Bolker B, Walker S (2015). "Fitting Linear Mixed-Effects Models Using lme4." Journal of Statistical Software, 67(1), 1–-48. doi:10.18637/jss.v067.i01.
...
[6] Fai AH, Cornelius PL (1996). "Approximate F-Tests of Multiple Degree of Freedom Hypotheses in Generalised Least Squares Analyses of Unbalanced Split-Plot Experiments."" Journal of Statistical Computation and Simulation, 54(4), 363–378. doi:10.1080/00949659608811740.
